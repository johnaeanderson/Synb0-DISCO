function datRGBtriDWMRI(Dmprspace, mpr, name, dataset, root)
...
% changed from load_nii
	...
	rmpr =  [Dmprspace filesep name '-r-mpr.nii.gz'];

  system(['fslreorient2std ' mpr ' ' rmpr]);
	...
	if(1)
	    system(['flirt ' ...
	        ...
	        '-ref ' atlas ' ' ...
	        '-out ' mprinmni ' ' ...
	        '-omat ' Dmprspace filesep name '-mpr-mni.mat ' ])
	end


	%% Step 4: Normalize the T1s to the MNI contrast
